...
async function cargarSettings() {
    try {
        ...
        if (cartaEl) cartaEl.textContent = data.carta || '';

        // Frases
        _frases = Array.isArray(data.frases) && data.frases.length ? data.frases : ['Eres increíble. ✨'];
        mostrarFrase(0);
    } catch {
        const cartaEl = document.getElementById('cartaTexto');
        if (cartaEl) cartaEl.textContent = '¡Hola! Hice esta pequeña página para recordarte lo importante que eres en mi vida. ¡Te quiero muchísimo!';
        _frases = ['Eres una persona increíble, valiente y divertida. ¡Nunca cambies! ✨'];
        mostrarFrase(0);
    }
}
